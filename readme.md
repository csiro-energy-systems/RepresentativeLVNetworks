...
The low voltage feeder taxonomy test case data is stored in `/data` and can be used straight away in OpenDSS. If you know how to use OpenDSS, you don't need to use this package. For windows, you can download OpenDSS at https://www.epri.com/pages/sa/opendss. On mac/linux, you can access OpenDSS' engine through either Python or Julia wrappers:
...
- Pluto.jl notebook files (in `notebooks`) that explore: 
    ...
    - Note that Pluto.jl notebooks is a Julia package that provides a convenient, user-friendly, interactive notebook user interface to Julia. The contents of the notebook files 1)-4) above provide guided, step-by-step, examples of OpenDSS analysis of the LVFT `/data` files.   
...
Installation instructions of RepresentativeLVNetworks.jl
 ...
 6) To confirm the successful establishment of the environment, typing 
 `Pkg.status() [enter]` at the "julia>" prompt in the terminal window should return 
